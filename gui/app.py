...
app.grid_columnconfigure(0, weight=0, minsize=280)
...
try:
    icon_map = {
        "add": "add_icon.png", "folder": "folder_icon.png", "settings": "settings_icon.png",
        "s_pending": "status_pending.png", 
        "s_processing": "status_processing.png",
        "s_downloading": "status_downloading.png", 
        "s_completed": "status_completed.png",
        "s_error": "status_error.png"
    }
    loaded_images = {}
    for k, fn in icon_map.items():
        p = resource_path_gui(os.path.join('assets', 'icons', fn))
        if os.path.exists(p):
            icon_size = (18, 18) if k.startswith("s_") else (20, 20) 
            pil_img = Image.open(p)
            loaded_images[k] = ctk.CTkImage(light_image=pil_img, dark_image=pil_img, size=icon_size)
        else:
            print(f"Icono no encontrado: {p}")
            
    img_add_queue = loaded_images.get("add")
    img_open_folder = loaded_images.get("folder")
    img_settings = loaded_images.get("settings")
    img_status_pending = loaded_images.get("s_pending")
    img_status_processing = loaded_images.get("s_processing", img_status_pending) 
    img_status_downloading = loaded_images.get("s_downloading")
    img_status_completed = loaded_images.get("s_completed")
    img_status_error = loaded_images.get("s_error")
...
video_duracion_actual_var = ctk.StringVar(value="Duración: -")
opciones_calidad_audio_map_app = { 
    "128 kbps (Estándar)":"128","192 kbps (Buena)":"192",
    "256 kbps (Alta)":"256","320 kbps (Máxima)":"320"
}
calidad_audio_display_var_app = ctk.StringVar(value=list(opciones_calidad_audio_map_app.keys())[1])

# --- Placeholders para widgets ---
info_carpeta_descargas_label = None 
etiqueta_imagen_actual = None 
frame_estado_y_acciones_actual_dm = None 
estado_label_actual_dm = None 
entrada_url_dm = None 
queue_scrollable_area_dm = None 
barra_progreso_actual = ctk.CTkProgressBar(app, height=15) 
...
class ToolTip: # ... (código de ToolTip como antes, sin cambios)
    def __init__(self, widget, text_func, delay=600, **kwargs):
        ...
        self.kwargs = kwargs; self.widget.bind("<Enter>", self.on_enter, add="+")
        self.widget.bind("<Leave>", self.on_leave, add="+"); self.widget.bind("<ButtonPress>", self.on_leave, add="+") 
...
def hilo_cargar_imagen_bytes(url_imagen, etiqueta_destino_widget_ref): # Sin cambios
    try:
        ...
        if not raw_data:
            if etiqueta_destino_widget_ref: app.after(0, lambda: (etiqueta_destino_widget_ref.configure(image=None, text="Miniatura vacía"), setattr(etiqueta_destino_widget_ref, 'image_ref', None) ))
            return
        ...
        target_width = etiqueta_destino_widget_ref.cget("width"); 
        if target_width < 20: target_width = 440 
        ...
        if new_h == 0 : new_h = int(new_w * 0.5625)
...
def abrir_ventana_configuracion_ui_cmd(): # Sin cambios
    ...
    baf = ctk.CTkFrame(mf,fg_color="transparent"); baf.grid(row=4,column=0,columnspan=3,pady=(20,0),sticky="s"); mf.grid_rowconfigure(3,weight=1); mf.grid_columnconfigure(1,weight=1)
...
ui_elements_for_manager = {
    ...
    "video_titulo_actual_var": video_titulo_actual_var, "video_duracion_actual_var": video_duracion_actual_var,
    "etiqueta_imagen_actual": None, "barra_progreso_actual": barra_progreso_actual, 
    "frame_elementos_cola": None, "calidad_audio_display_var": calidad_audio_display_var_app, 
