...
import sys, os, platform, subprocess, threading, queue, urllib.request, urllib.error
...
from gui.config_utils import resource_path_gui, APP_NAME, CARPETA_MUSICA, cargar_configuracion_inicial, guardar_configuracion_actual, CONFIG_DEFAULTS, APP_VERSION, verificar_actualizaciones_en_hilo
from gui.download_manager import DownloadManager

cargar_configuracion_inicial()
app = ctk.CTk()
app.title(f"{APP_NAME} v{APP_VERSION}"); app.geometry("850x700"); app.minsize(600, 700)
...
img_status_pending, img_status_processing, img_status_downloading, img_status_completed, img_status_error = None, None, None, None, None
try:
...
# --- LAYOUT DE LA INTERFAZ GRÁFICA PRINCIPAL ---
...
ui_elements_for_manager = {
    ...
    "tipo_descarga_var": tipo_descarga_switch_var 
}
...
frame_gestion_cola = ctk.CTkFrame(sidebar_frame, fg_color="transparent")
frame_gestion_cola.grid(row=2, column=0, padx=10, pady=(10,0), sticky="ew")
frame_gestion_cola.grid_columnconfigure(0, weight=1); frame_gestion_cola.grid_columnconfigure(1, weight=0)
ctk.CTkLabel(frame_gestion_cola, text="Cola de Descargas:", font=("Arial", 13, "bold")).grid(row=0, column=0, sticky="sw", pady=(0,2))
boton_limpiar_cola = ctk.CTkButton(frame_gestion_cola, text="Limpiar Pendientes", height=28, font=("Arial", 10), width=100, command=download_mgr.limpiar_cola_pendientes_async) 
boton_limpiar_cola.grid(row=0, column=1, sticky="se", padx=(5,0), pady=(0,2))
...
def iniciar_verificacion_actualizaciones():
    if download_mgr and hasattr(download_mgr, 'ui') and download_mgr.ui.get("estado_general_actual_var") :
        threading.Thread(target=verificar_actualizaciones_en_hilo, args=(app, download_mgr.ui), daemon=True).start()
